...
template<typename T> __forceinline bool casP(T *volatile *ptr,T *oldV,T *newV) {return InterlockedCompareExchangePointer(ptr,newV,oldV)==oldV;}
template<typename T> __forceinline T casV(volatile T *ptr,T oldV,T newV) {return InterlockedCompareExchange(ptr,newV,oldV);}
inline long InterlockedDecrement(volatile long * pValue)
{
volatile long lRes;
   __asm__ __volatile__ (
	    ...
	    "       sub   r2, r2, #1   \n" 
		"       mov %0, r2		   \n"
		" strex r4, r2, %1 \n"  //update *pValue and unlock the bus...
		"       teq r4, #0         \n"  
		"       bne L_%="
	 	: "=&r" (lRes)
		: "r"  (pValue)
...
inline long InterlockedIncrement(volatile long *pValue)
{
volatile long lRes;
     __asm__ __volatile__ (
	    "L_%=:" " ldrex r2, %1      \n"  //lock the bus
...
inline long InterlockedCompareExchange(volatile long *pValue, long pNew, long pOld)
{
	bool lRes; long old;
	__asm__ __volatile (
		"mov %0, #1\n"
		...
		"E_%=: \n"
		: "=&r" (lRes), "=&r" (old)
		: "r" (pValue), "Ir" (pOld), "r" (pNew)
		: "cc", "memory");
	return lRes ? old : pOld;
} 

inline void *InterlockedCompareExchangePointer(void* volatile *pValue, void *pNew, void *pOld)
{
	bool lRes; void * old;
	__asm__ __volatile (
		"mov %0, #1\n"
...
inline int cas64(volatile uint64_t* destination,
                 volatile uint64_t* comperand,
                 volatile uint64_t* exchange)
{
int lRes;  
   __asm__ __volatile__
   (
...
template<unsigned blockSize=0x200> class FreeQ
{
	SLIST_HEADER	qfree;
	SLock			lock;
	const HEAP_TYPE	allc;
public:
	FreeQ(HEAP_TYPE al) : allc(al) {InitializeSListHead(&qfree);}
	void *alloc(size_t s) {
		...
		if (se==NULL) {
			...
			s=s+15&~15;
			if ((se=InterlockedPopEntrySList(&qfree))==NULL && (se=(SLIST_ENTRY*)memalign(16,blockSize*s,allc))!=NULL)
#else
			s=s+7&~7;
			if ((se=InterlockedPopEntrySList(&qfree))==NULL && (se=(SLIST_ENTRY*)malloc(blockSize*s,allc))!=NULL)
#endif
