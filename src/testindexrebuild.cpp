...
void TestIndexRebuild::createFamilies()
{
	...
	for(i = 0; i < sNumFamiliesPerType; i++)
	{
		...
		IExprNode *lET;
		{
			...
			lV[0].setVarRef(0,mPropIDs[0]);
			lV[1].set(30);
			IExprNode *lET1 = mSession->expr(OP_GT, 2, lV);
			lV[0].setVarRef(0,mPropIDs[0]);
			lV[1].set(40);
			IExprNode *lET2 = mSession->expr(OP_LT, 2, lV);
			lV[0].set(lET1);
			lV[1].set(lET2);
			IExprNode *lET3 = mSession->expr(OP_LAND, 2, lV);
			lV[0].setVarRef(0,mPropIDs[1]);
			lV[1].setParam(0);
			IExprNode *lET4 = mSession->expr(OP_IN, 2, lV);
			lV[0].set(lET3);
			lV[1].set(lET4);
...
void TestIndexRebuild::createClasses()
{
	...
	for(i = 0; i < sNumClassesPerType; i++)
	{
		...
		IExprNode *lET;
		{
			...
			lV[0].setVarRef(0,mPropIDs[0]);
			lV[1].set(50);
			IExprNode *lET1 = mSession->expr(OP_GE, 2, lV);
			lV[0].setVarRef(0,mPropIDs[1]);
