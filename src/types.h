...
inline	ulong	getPageSize() {SYSTEM_INFO si; GetSystemInfo(&si); return si.dwPageSize;}
...
inline	void	*allocAligned(size_t sz,ulong) {return VirtualAlloc(NULL,sz,MEM_COMMIT,PAGE_READWRITE);}
...
__forceinline size_t ceil(size_t l,ulong alignment) {assert((alignment&alignment-1)==0); return l + alignment - 1 & ~(alignment - 1);}
__forceinline size_t align(size_t sht,ulong alignment) {assert((alignment&alignment-1)==0); return (sht + alignment - 1 & ~(alignment - 1)) - sht;}
__forceinline size_t floor(size_t l,ulong alignment) {assert((alignment&alignment-1)==0); return l & ~(alignment - 1);}
